...
public:
    ...
    void operator()(float linkLength , int  particleSize)
    {
        ...
        std::cout << "Number of Particles : " << numOfParticles <<  " Number of Halos found : " << numOfHalos <<  "\n" << endl;
    }
    ...
    void getBalancedKdTree()
    {
        ...
        kd_tree.resize(numOfParticles*2); //kd_tree = thrust::device_vector<KDtreeNode>(numOfParticles*2);

        //set the size for the level details array
        levelInfo.resize(levels); //levelInfo =thrust::device_vector<Level>(levels);
        ...
        //thrust::transform(kd_tree.begin()+(count-1), kd_tree.begin()+count, kd_tree.begin()+(count-1), setNodeValues(0,-1,-1,0,numOfParticles));
	  ...
	  lChildren.resize(size); //thrust::device_vector<KDtreeNode> lChildren(size);
	  rChildren.resize(size); //thrust::device_vector<KDtreeNode> rChildren(size);
	...
	for (int l=levels-2; l>=0; l--)
	{
	  ...
	  //---------
	  //comment this do all of the n*n comparisons instead of m*n
	  {
