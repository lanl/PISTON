...
//	std::cout << "num total vertices: " << numTotalVertices << std::endl;
	...
	__host__ __device__
	float3 vertexInterp(float isolevel, float3 p0, float3 p1, float f0, float f1) const {
	    const float t = (isolevel - f0) / (f1 - f0);
	    return lerp(p0, p1, t);
	}
	__host__ __device__
	float scalarInterp(float isolevel, float s0, float s1, float f0, float f1) const {
	    const float t = (isolevel - f0) / (f1 - f0);
	    return lerp(s0, s1, t);
	}
	...
	void operator()(thrust::tuple<int, int, int, int> indices_tuple) {
		...
		*(vertices_output + outputVertId + v) = make_float4(vertexInterp(isovalue, p[v0], p[v1], f[v0], f[v1]), 1.0f);
		*(scalars_output  + outputVertId + v) = scalarInterp(isovalue, s[v0], s[v1], f[v0], f[v1]);
	    }
